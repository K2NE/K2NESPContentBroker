...
$guid = [guid]"EBAEF90D-3657-4A49-86F1-7905AB0E2EA2" 
$brokerName = "K2Field.K2NE.SPContentBroker"
$brokerSystemName = $brokerName
$brokerDLL = "K2Field.K2NE.SPContentBroker.dll"
$displayName = $brokerName -replace '\.',' '
$brokerDescription = "The K2NE SP Content Broker."
...
Function RegisterServiceType([string]$k2ConnectionString, [string]$k2Server, [string]$ServiceTypeDLL, [guid]$guid, [string]$systemName, [string]$displayName, [string]$description = "") {
    ...
    $smoManServiceAssembly = Join-Path $targetPath "bin\SourceCode.SmartObjects.Services.Management.dll"
    $serviceBrokerAssembly = Join-Path $targetPath "ServiceBroker\$($ServiceTypeDLL)"
	$className = "K2Field.K2NE.SPContentBroker.SPContentBroker";
...
Copy-Item $scriptPath\* -Include "Microsoft.SharePoint.Client.Taxonomy.dll" -Destination "$targetPath\ServiceBroker"
